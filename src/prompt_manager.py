...
## 合规内容判断标准（重要）
以下情况应当判定为合规（compliant=yes）：
- 内容包含正面评价但无违规表述（如"保障全面""价格实惠"等主观感受），且引导消费者阅读条款
- 内容提及收益数据但同时包含风险提示（如"过往业绩不代表未来表现""保单利益具有不确定性"）
- 内容标明价格、承保机构、引导阅读条款，无违规用语
- 内容客观描述产品特征，未使用绝对化用语、未承诺收益、未混淆产品属性
- 仅因内容简短或缺少某些信息不构成违规，只有明确违反法规禁止性条款才算违规
